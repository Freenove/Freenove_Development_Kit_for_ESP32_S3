...
#include "lvgl.h"
#include "lv_example_btnmatrix.h"
...
static void event_cb_2(lv_event_t * e)
{
  ...
  if (code == LV_EVENT_DRAW_PART_BEGIN) {
    ...
    /*Change the draw descriptor the 3rd button*/
      ...
      dsc->rect_dsc->radius = 15;
      if (lv_btnmatrix_get_selected_btn(obj) == dsc->id)  dsc->rect_dsc->bg_color = lv_palette_darken(LV_PALETTE_YELLOW, 3);
      else dsc->rect_dsc->bg_color = lv_palette_main(LV_PALETTE_YELLOW);
...
void lv_example_btnmatrix_2(void)
{
  ...
  lv_obj_add_event_cb(btnm, event_cb_2, LV_EVENT_ALL, NULL);
  lv_obj_center(btnm);
}
...
void lv_example_btnmatrix_3(void)
{
  ...
  lv_obj_add_event_cb(btnm, event_cb_3, LV_EVENT_VALUE_CHANGED, NULL);
